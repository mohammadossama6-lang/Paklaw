...
/* min-h-11 keeps <select> the same height as <input>: browsers give selects
   `line-height: normal` regardless of the text size, which otherwise left every
   dropdown 4px shorter than the fields beside it. It also puts each control on
   a 44px touch target. */
export const inputClass =
  "w-full min-h-11 rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-[15px] text-ink shadow-sm outline-none transition-all placeholder:text-slate-400 hover:border-brand-300 focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-400 disabled:shadow-none";
